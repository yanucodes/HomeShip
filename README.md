...
**End-to-end API smoke test** — with the server running, `./scripts/api_smoke_test.sh` drives the full user → ship → task → supply lifecycle over HTTP and asserts the responses. It then adds a second user to the ship to verify shared-crew visibility — both members see identical task and supply lists, a deletion by either is visible to both, and a member who leaves loses access while the ship stands for the rest. Requires [`httpie`](https://httpie.io) and [`jq`](https://jqlang.github.io/jq); target a remote instance by passing the base URL as an argument (or via `BASE_URL=...`). CI runs this against the containerized stack on every push.
