...
def login(form_data: OAuth2PasswordRequestForm = Depends(),
    ...
    Args:
        form_data: OAuth2 password-flow form. Per the OAuth2 spec the login
            field is named `username`, but this app accepts either an email
            or a username there: `form_data.username` carries whichever the
            user supplied, and the service distinguishes them by the '@'.
    ...
    Raises:
        HTTPException: 401 if no user matches the identifier or the password
            is wrong.
    """
